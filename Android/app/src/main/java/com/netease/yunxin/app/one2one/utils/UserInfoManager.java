...
public class UserInfoManager {
    ...
    public static void getUserInfoByImAccId(String imAccId, NECallback<NimUserInfo> callback) {
        NIMClient.getService(UserService.class).fetchUserInfo(Collections.singletonList(imAccId))
                .setCallback(new RequestCallback<List<NimUserInfo>>() {
                    ...
                    public void onException(Throwable exception) {
                        callback.onError(ErrorCode.ERROR, "Failed to fetch userInfo,exception=" + exception);
                    }
                });
    }
