...
public class PstnCallViewModel extends AndroidViewModel {
    ...
    private NERtcCallbackExTemp rtcCallback = new NERtcCallbackExTemp() {
        @Override
    ...
    private AbsPstnCallback pstnCallback = new AbsPstnCallback() {
        ...
        public void onRtcCallResult(@NonNull ResultInfo<ChannelFullInfo> result) {
            ...
            if (result.getMsg() != null && result.getMsg().getCode() == ResponseCode.RES_PEER_NIM_OFFLINE) {
                return;
            }
    ...
    public void startInTheCallTimer() {
        ...
        inThePstnCallSecondTimer.start(new Function1<Long, Unit>() {
            @Override
    ...
    protected void onCleared() {
        ...
        cancelInTheTimer();
        AccountAmountHelper.addPstnUsedDurationWithAccount(UserInfoManager.getSelfImAccid(), pstnCallUsedDuration);
        hangup = false;
    }
