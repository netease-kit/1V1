...
public class AssetUtils {
    ...
    public static int copyAssetRecursive(AssetManager manager, String src, String dest, boolean overwrite) {
        int ret = 0;
        if (manager == null) {
            ALog.e(TAG, "AssetManager is null");
            ret = -1;
        ...
        try {
            ...
            if (list.length == 0) {
                ...
                for (String path : list) {
                    copyAssetRecursive(manager, src + File.separator + path, dest + File.separator + path, overwrite);
                }
            }
