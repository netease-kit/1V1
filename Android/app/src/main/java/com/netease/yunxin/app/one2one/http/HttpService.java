...
public class HttpService {
    public static Single<BaseResponse> requestRtcToken(long uid) {
        ServerApi serverApi = NetworkClient.getInstance().getService(ServerApi.class);
        Map<String, Object> map = new HashMap<>();
        map.put("roomCname", "");
        map.put("uid", uid);
        return serverApi.requestRtcToken(map).compose(scheduleThread())
                .map(string -> string);
    }
    ...
    public static Single<BaseResponse<UserModel>> searchUserInfoWithPhoneNumber(String phoneNumber) {
        ...
        return serverApi.searchUserWithPhoneNumber(map).compose(scheduleThread())
    ...
    public static Single<BaseResponse> sendSms(String phoneNumber,String noticeUser) {
        ...
        return serverApi.sendSms(map).compose(scheduleThread())
