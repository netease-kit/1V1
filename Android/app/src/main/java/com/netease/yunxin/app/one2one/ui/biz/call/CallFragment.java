...
public class CallFragment extends Fragment {
    ...
    public void onAttach(@NonNull Context context) {
        ...
        OnBackPressedCallback callback = new OnBackPressedCallback(
            ...
            public void handleOnBackPressed() {
                new AlertDialog.Builder(activity).setTitle(activity.getString(R.string.end_call)).setMessage(activity.getString(R.string.sure_end_call)).setPositiveButton(activity.getString(R.string.yes), new DialogInterface.OnClickListener() {
                    @Override
        ...
        try {
            ...
            if (!callParams.isCalled()) {
                ...
                if (callParams.getChannelType() == ChannelType.AUDIO.getValue() && needPstnCall) {
                    ...
                    CallTimeOutHelper.configTimeOut(CallConfig.CALL_TOTAL_WAIT_TIMEOUT, CallConfig.CALL_PSTN_WAIT_MILLISECONDS);
                    ...
                    activity.rtcCall(new NECallback<ChannelFullInfo>() {
                        @Override
    ...
    private void handleSmsEvent() {
        viewModel.getSendSmsData().observe(getViewLifecycleOwner(), new Observer<Boolean>() {
            @Override
    ...
    private void handlePstnEvent() {
        ...
        pstnCallViewModel.getSwitchToInTheCall().observe(viewLifecycleOwner, new Observer<Integer>() {
            @Override
        ...
        pstnCallViewModel.getPstnToastData().observe(viewLifecycleOwner, new Observer<String>() {
            @Override
        ...
        pstnCallViewModel.getRtcCallResult().observe(viewLifecycleOwner, new Observer<Boolean>() {
            @Override
            public void onChanged(Boolean aBoolean) {
                callFinished = aBoolean;
            }
        });
        pstnCallViewModel.getSendSmsData().observe(viewLifecycleOwner, new Observer<Boolean>() {
            @Override
            public void onChanged(Boolean aBoolean) {
                sendSms();
            }
    ...
    private void handleCallFinishedEvent() {
        viewModel.getCallFinished().observe(getViewLifecycleOwner(), new Observer<Boolean>() {
            @Override
            public void onChanged(Boolean aBoolean) {
                finishActivity();
            }
    ...
    private void handleRingEvent() {
        viewModel.getPlayRing().observe(getViewLifecycleOwner(), new Observer<AVChatSoundPlayer.RingerTypeEnum>() {
            @Override
    ...
    private void handleToastEvent() {
        viewModel.getToastData().observe(getViewLifecycleOwner(), new Observer<String>() {
            @Override
            public void onChanged(String s) {
    ...
    private void handleOtherInfoUi() {
        viewModel.getOtherInfo().observe(getViewLifecycleOwner(), new Observer<OtherUserInfo>() {
            @Override
            public void onChanged(OtherUserInfo otherUserInfo) {
                if (otherUserInfo.callType == ChannelType.VIDEO.getValue()) {
                    ...
                    binding.ivInvitedAccept.setBackgroundResource(R.drawable.selector_img_video_accept);
                } else {
                    binding.clRoot.setBackgroundResource(R.drawable.bg_audio_call_page);
                    binding.ivInvitedAccept.setBackgroundResource(R.drawable.selector_img_audio_accept);
                }
    ...
    private void initEvent() {
        ...
        binding.ivInviteCancel.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        binding.ivInvitedAccept.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
        ...
        binding.ivInvitedReject.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                handleHangupEvent();
            }
    ...
    private void handleHangupEvent() {
        if (needPstnCall && callParams.getChannelType() == ChannelType.AUDIO.getValue()) {
            ...
            activity.rtcHangup(new NECallback<Integer>() {
                @Override
    ...
    private void releaseAndFinish(boolean finishCall) {
        LogUtil.i(TAG, "releaseAndFinish,finishCall:" + finishCall + ",isCalled:" + callParams.isCalled());
        AVChatSoundPlayer.Companion.instance().stop(AVChatSoundPlayer.RingerTypeEnum.RING, activity);
        AVChatSoundPlayer.Companion.instance().stop(AVChatSoundPlayer.RingerTypeEnum.CONNECTING, activity);
        if (finishCall) {
            if (callParams.isCalled()) {
                activity.rtcHangup(new NECallback<Integer>() {
                    @Override
