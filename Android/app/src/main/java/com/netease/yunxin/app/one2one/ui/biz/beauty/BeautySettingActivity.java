...
import com.netease.yunxin.app.one2one.databinding.ActivityBeautySetting2Binding;
...
import com.netease.yunxin.app.one2one.ui.view.MyRadioButton;
import com.netease.yunxin.app.one2one.ui.view.NEBeautyRadioGroup;
...
public class BeautySettingActivity extends BaseActivity {
    ...
    private ActivityBeautySetting2Binding binding;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        binding = ActivityBeautySetting2Binding.inflate(getLayoutInflater());
    ...
    private void initViews() {
        ImmersionBar.with(this)
                ...
                .init();
        binding.imgBeautyCompared.setOnTouchListener((view, motionEvent) -> {
        ...
        @SuppressLint("InflateParams") View effectTab = getLayoutInflater().inflate(R.layout.tab_skin_effect, null);
        tabViews.add(effectTab);
        @SuppressLint("InflateParams") View bodyEffectTab = getLayoutInflater().inflate(R.layout.tab_body_effect, null);
        tabViews.add(bodyEffectTab);
        @SuppressLint("InflateParams") View filterTab = getLayoutInflater().inflate(R.layout.tab_filter, null);
        ...
        recoveryRadioButton.setOnClickListener(v -> {
            ...
            if (currentEffect.get() == DEFAULT_SELECT_EFFECT) {
                effectLevelSlider.setValue(
                        BeautyManager.getInstance().getDefaultEffects().get(DEFAULT_SELECT_EFFECT).getLevel() * 100);
            }
        ...
        NEBeautyRadioGroup bodyEffectRadioGroup = bodyEffectTab.findViewById(R.id.radio_group_body_effect);
        MyRadioButton bodyRecoveryRadioButton = bodyEffectTab.findViewById(R.id.rb_effect_recover);
        bodyRecoveryRadioButton.setOnClickListener(v -> {
            ...
            if (currentBodyEffect.get() == DEFAULT_SELECT_BODY_EFFECT) {
                bodyEffectLevelSlider.setValue(
                        BeautyManager.getInstance().getDefaultBodyEffects().get(DEFAULT_SELECT_BODY_EFFECT).getLevel() * 100);
            }

        });
        bodyEffectRadioGroup.setOnCheckedChangeListener((radioGroup, checkedId) -> {
        ...
        bodyEffectRadioGroup.check(DEFAULT_SELECT_BODY_EFFECT);
        bodyEffectLevelSlider.addOnChangeListener((slider, value, fromUser) -> {
            float level = value / 100;
        ...
        filterRecoveryRadioButton.setOnClickListener(v -> {
        ...
        binding.vpPager.setAdapter(new PagerAdapter() {
        ...
        binding.beautyOk.setOnClickListener(v -> {
            saveData();
            finish();
        });
        binding.openBeauty.setOnClickListener(v -> showOrDismissBeautySettingPanel());
        binding.vvLocalUser.setOnClickListener(v -> {
