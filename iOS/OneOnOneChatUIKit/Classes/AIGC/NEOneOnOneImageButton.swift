...
private class NEOneOnOneImageButtonProps {
  var title: String?
  var image: UIImage?
}

/// 图标在上，文字在下的通用按钮
public class NEOneOnOneImageButton: UIControl {
  private var stateProps = [UInt: NEOneOnOneImageButtonProps]()
  ...
  public func setTitle(_ title: String?, for state: UIControl.State) {
    if let props = stateProps[state.rawValue] {
      ...
      let props = NEOneOnOneImageButtonProps()
      props.title = title
  ...
  private func updateUI(props: NEOneOnOneImageButtonProps) {
