...
struct NEOneOnOneDecoder {
  ...
  static func decode<T>(_ type: T.Type, data: Data) -> T? where T: Decodable {
    ...
    do {
      ...
      NEOneOnOneLog.errorLog(tag, desc: "Failed to decode.")
    }
    return model
  }
}
...
/// 编码器
struct NEKaraokeEncoder {
  let tag: String = "NEKaraokeEncoder"

  // MARK: - ------------------------- 模型转json字符串 --------------------------

  internal static func encoder<T>(toString model: T) -> String? where T: Encodable {
  ...
  internal static func encoder<T>(toDictionry model: T) -> [String: Any]? where T: Encodable {
    let encoder = JSONEncoder()
    encoder.outputFormatting = .prettyPrinted
    guard let data = try? encoder.encode(model) else {
      return nil
