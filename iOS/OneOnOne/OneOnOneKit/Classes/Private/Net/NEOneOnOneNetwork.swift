...
let NE = NEOneOnOneNetwork.shared

/// 网络请求类
public class NEOneOnOneNetwork {
  let tag: String = "NEOneOnOneNetwork"
  static let shared = NEOneOnOneNetwork()
  // 配置信息
  ...
  func request(_ url: String,
    ...
    let dataTask = session.dataTask(with: request) { [weak self] data, response, error in
      ...
      if let error = error as NSError? {
        ...
        failed(makeError(NEOneOnOneErrorCode.failed, "网络连接失败，请检查你的网络连接！"))
        return
      }

      guard let httpResp = response as? HTTPURLResponse else {
        failed(makeError(
          NEOneOnOneErrorCode.failed,
      ...
      guard httpResp.statusCode == 200 else {
        ...
        failed(makeError(NEOneOnOneErrorCode.failed,
                         "Http error with status: \(httpResp.statusCode)"))
        return
      }
      ...
      guard let response = try? JSONSerialization.jsonObject(
        ...
        failed(makeError(NEOneOnOneErrorCode.failed, "Cannot decode content data"))
        return
      }
      // 打印requestId
      ...
      NEOneOnOneLog.infoLog(self.tag, desc: "Response data:\n\(response.prettyJSON)")
      success(dic)
    }
    dataTask.resume()
  }
  ...
  func request<T>(_ url: String,
    ...
    request(url, method: method, headers: headers, body: body) { dic in
      ...
      success(NEOneOnOneDecoder.decode(returnType, param: dic) ?? nil)
    } failed: { error in
      failed(error)
    }
  }
