...
@objcMembers
class _NEOneOnOneCustomMessageDataAudio: NSObject, Codable {
  // 违规内容
  var content: String = ""
  // 检测结果，1表示嫌疑，2表示不通过
  var action: Int = -1
...
@objcMembers
class _NEOneOnOneCustomMessageDataVideo: NSObject, Codable {
  // 证据类型，1：图片，2：视频
  var type: Int = 0
  // 证据信息,该字段与审核事件字段保持一致
  var url: String = ""
...
@objcMembers
class _NEOneOnOneAccountInfo: NSObject, Codable {
  // im账号
  var imToken: String = ""
  // 头像
  var icon: String = ""
...
@objcMembers
class _NEOneOnOneOnlineUserList: NSObject, Codable {
  var data: [_NEOneOnOneOnlineUser]?
}

@objcMembers
class _NEOneOnOneOnlineUser: NSObject, Codable {
  // 用户信息
  var userUuid: String?
  // 昵称
  var userName: String?
