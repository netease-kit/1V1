...
public enum NPTReachabilityError: Error {
...
public extension Notification.Name {
  static let reachabilityChanged = Notification.Name("nptReachabilityChanged")
}

public class NPTReachability {
  public typealias NetworkReachable = (NPTReachability) -> Void
  public typealias NetworkUnreachable = (NPTReachability) -> Void
  ...
  public required init(reachabilityRef: SCNetworkReachability,
    ...
    reachabilitySerialQueue = DispatchQueue(label: "com.party.reachability", qos: queueQoS, target: targetQueue)
  ...
  public convenience init(hostname: String,
    ...
    guard let ref = SCNetworkReachabilityCreateWithName(nil, hostname) else {
      throw NPTReachabilityError.failedToCreateWithHostname(hostname, SCError())
    }
  ...
  public convenience init(queueQoS: DispatchQoS = .default,
    ...
    guard let ref = SCNetworkReachabilityCreateWithAddress(nil, &zeroAddress) else {
      throw NPTReachabilityError.failedToCreateWithAddress(zeroAddress, SCError())
    }
...
public extension NPTReachability {
  ...
  func startNotifier() throws {
    ...
    if !SCNetworkReachabilitySetCallback(reachabilityRef, callback, &context) {
      stopNotifier()
      throw NPTReachabilityError.unableToSetCallback(SCError())
    }

    if !SCNetworkReachabilitySetDispatchQueue(reachabilityRef, reachabilitySerialQueue) {
      stopNotifier()
      throw NPTReachabilityError.unableToSetDispatchQueue(SCError())
    }
...
private extension NPTReachability {
  func setReachabilityFlags() throws {
    try reachabilitySerialQueue.sync { [unowned self] in
      var flags = SCNetworkReachabilityFlags()
      if !SCNetworkReachabilityGetFlags(reachabilityRef, &flags) {
        stopNotifier()
        throw NPTReachabilityError.unableToGetFlags(SCError())
      }
...
extension SCNetworkReachabilityFlags {
  typealias Connection = NPTReachability.Connection
...
private class ReachabilityWeakifier {
  weak var reachability: NPTReachability?
  init(reachability: NPTReachability) {
